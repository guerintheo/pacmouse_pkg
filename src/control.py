...
def step(x, sp):
	...
	Returns:
	    1d numpy array: a 2-vector of the commanded wheel speeds
	"""
	steering_coeff = 100.
	drive_coeff    = 1000.

	e_x, e_y = sp[:2] - x[:2] 					# the position difference
	e_t = np.arctan2(e_y, e_x) - x[2] 			# the angular error to point at the desired position
	...
	return mix([drive_command, steering_command])
...
def get_sp(x, maze, target_cell):
	...
	target_pos = (maze.index_to_xy(path[1]) + 0.5) * p.maze_inner_size
	return target_pos
