...
class GameState:
    ...
    def _die(self):
        self.score -= 1000  # maybe remove this
        if self.lives > 1:
    ...
    def _should_die(self):
        return ((self.red.pos[0] == self.pacbot.pos and self.red.frightened_counter == 0) or
        (self.pink.pos[0] == self.pacbot.pos and self.pink.frightened_counter == 0) or
        (self.orange.pos[0] == self.pacbot.pos and self.orange.frightened_counter == 0) or
        (self.blue.pos[0] == self.pacbot.pos and self.blue.frightened_counter == 0))
    ...
    def _check_if_ghost_eaten(self, ghost):
        if ghost.pos[0] == self.pacbot.pos and ghost.frightened_counter > 0:
            ghost.send_home()
